	...
	extends JVMObject
{
		...
		throws JVMClassCastException, JVMNegativeArraySizeException
	{
		...
		if (__type.isArray())
			throw new JVMClassCastException(String.format("LI0d %s %d", __type,
				__length));
		
		// {@squirreljme.error LI0e Attempting to create an array with a
		// negative length. (The negative length)}
		if (__length < 0)
			throw new JVMNegativeArraySizeException(String.format("LI0e %s",
				__length));
	...
	{
		// {@squirreljme.error LI0f An attempt was made to set the element of
