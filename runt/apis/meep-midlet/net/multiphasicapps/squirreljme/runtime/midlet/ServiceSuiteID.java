	...
	implements Comparable<ServiceSuiteID>
{
		...
		throws IllegalArgumentException, NullPointerException
	{
		...
		// {@squirreljme.error AD0m Expected a separator to appear in the
		// specified string. (The input string; The separator)}
		int cola = __s.indexOf(';');
		if (cola < 0)
			throw new IllegalArgumentException(String.format("AD0m %s %c",
				__s, ';'));
		
		// {@squirreljme.error AD0n Expected a second separator to appear in
		// the specified string. (The input string; The separator)}
		int colb = __s.indexOf(';', cola + 1);
		if (colb < 0)
			throw new IllegalArgumentException(String.format("AD0n %s %c",
				__s, ';'));
