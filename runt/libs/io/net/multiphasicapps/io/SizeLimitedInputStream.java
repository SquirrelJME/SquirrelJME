	...
	extends InputStream
{
		...
		throws IllegalArgumentException, NullPointerException
	{
		...
		// {@squirreljme.error BD0f The limit is negative. (The negative
		// limit)}
		if (__li < 0)
			throw new IllegalArgumentException(String.format("BD0f %d", __li));
		...
		throws IOException
	{
		...
		if (!this._closed)
		{
			...
			if (this.exact)
			{
				// {@squirreljme.error BD0f Reached EOF on wrapped stream when
