...
sjme_attrUnused SJME_TEST_DECLARE(testNvmLocalPopDouble)
{
	...
	stack = frame->stack;
	longsTread->values.d[1].hi = 0x12345678;
	longsTread->values.d[1].lo = 0x9ABCDEF0;
	...
	sjme_unit_equalI(test, 0x12345678, longsTread->values.d[0].hi,
		"Popped stack into local was not the correct value.");
	sjme_unit_equalI(test, 0x9ABCDEF0, longsTread->values.d[0].lo,
		"Popped stack into local was not the correct value.");
		
	/* And the stack value was cleared. */
	sjme_unit_equalI(test, 0, longsTread->values.d[1].hi,
		"Stack value did not get cleared.");
	sjme_unit_equalI(test, 0, longsTread->values.d[1].lo,
		"Stack value did not get cleared.");
