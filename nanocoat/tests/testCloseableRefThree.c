...
/**
 * Test closeable with three references.
 *  
 * @since 2024/09/28 
 */
SJME_TEST_DECLARE(testCloseableRefThree)
{
	...
	if (sjme_error_is(test->error = sjme_alloc_weakRef(closing, NULL)))
		return sjme_unit_fail(test, "Could not count 3?");
	...
	sjme_unit_equalI(test, 2, sjme_atomic_sjme_jint_get(&weak->count),
		"Wrong count?");
