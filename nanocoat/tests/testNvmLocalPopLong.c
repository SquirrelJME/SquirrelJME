...
sjme_attrUnused SJME_TEST_DECLARE(testNvmLocalPopLong)
{
	...
	stack = frame->stack;
	longsTread->values.j[1].part.hi = 0x12345678;
	longsTread->values.j[1].part.lo = 0x9ABCDEF0;
	...
	sjme_unit_equalI(test, 0x12345678, longsTread->values.j[0].part.hi,
		"Popped stack into local was not the correct value.");
	sjme_unit_equalI(test, 0x9ABCDEF0, longsTread->values.j[0].part.lo,
		"Popped stack into local was not the correct value.");
		
	/* And the stack value was cleared. */
	sjme_unit_equalI(test, 0, longsTread->values.j[1].part.hi,
		"Stack value did not get cleared.");
	sjme_unit_equalI(test, 0, longsTread->values.j[1].part.lo,
		"Stack value did not get cleared.");
