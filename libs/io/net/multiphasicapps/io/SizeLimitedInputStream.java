	...
	extends InputStream
{
		...
		throws IllegalArgumentException, NullPointerException
	{
		...
		// {@squirreljme.error BD0a The limit is negative. (The negative
		// limit)}
		if (__li < 0)
			throw new IllegalArgumentException(String.format("BD0a %d", __li));
		...
		throws IOException
	{
		...
		if (!this._closed)
		{
			...
			if (this.exact)
			{
				// {@squirreljme.error BD0b Reached EOF on wrapped stream when
				...
				while ((at = this._current) < limit)
					if (read() < 0)
						throw new IOException(String.format("BD0b %d %d",
							at, limit));
			}
		}
		...
		throws IOException
	{
		...
		if (next < 0)
		{
			// {@squirreljme.error BD0c Required an exact number of bytes
