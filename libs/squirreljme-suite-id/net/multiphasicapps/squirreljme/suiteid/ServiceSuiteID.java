	...
	implements Comparable<ServiceSuiteID>
{
		...
		throws IllegalArgumentException, NullPointerException
	{
		...
		// {@squirreljme.error CC0c Expected a separator to appear in the
		// specified string. (The input string; The separator)}
		int cola = __s.indexOf(';');
		if (cola < 0)
			throw new IllegalArgumentException(String.format("CC0c %s %c",
				__s, ';'));
		
		// {@squirreljme.error CC0d Expected a second separator to appear in
		// the specified string. (The input string; The separator)}
		int colb = __s.indexOf(';', cola + 1);
		if (colb < 0)
			throw new IllegalArgumentException(String.format("CC0d %s %c",
				__s, ';'));
