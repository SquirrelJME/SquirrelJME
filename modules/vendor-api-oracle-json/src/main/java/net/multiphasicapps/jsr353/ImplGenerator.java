	...
	implements JsonGenerator
{
	...
	public JsonGenerator writeEnd()
	{
		synchronized (this._lock)
		{
			...
			char e = (this._ss.get(0) == GeneratorScope.ARRAY ? ']' : '}');
	...
	private JsonGenerator statelessStartX(String __n, GeneratorScope __s)
	{
		...
		if (__n != null)
			this.putKeyName(__n);
		this.__print((__s == GeneratorScope.ARRAY ? '[' : '{'));
		if (this._iw != null)
		{
			this._iw.addIndent(ImplGenerator.INDENT);
			this.__printLn();
	...
	{
		// Must be in object
		if (this._done || this._ss.isEmpty() || this._ss.get(
			0) != GeneratorScope.OBJECT)
